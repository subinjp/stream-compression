...
class Node_build(object):
    recurPrint = False
    def __init__(self, ch=None, fq=None, left_node=None, right_node=None, parent=None):
       self.fq = fq
       self.L = left_node
       self.R = right_node
       self.p = parent
    ...
    def __repr__(self):
        if Node_build.recurPrint:
            left_node = self.L if self.L else '#'
            right_node = self.R if self.R else '#'
            return ''.join( ('(%s:%d)'%(self.c, self.fq), str(left_node), str(right_node) ) )
        else:
            return '(%s:%d)'%(self.c, self.fq)

    def __cmp__(self, other):
        if not isinstance(other, Node_build):
            return super(Node_build, self).__cmp__(other)
        return cmp(self.fq, other.fq)

def get_two_nodes(nodes):
    if len(nodes)>1:
        first=nodes.pop(0)
        second=nodes.pop(0)
        return first, second
...
def tree_build(nodes):
    nodes.sort()
    while(True):
        first, second = get_two_nodes(nodes)
        if not second:
            return first
        parent = Node_build(left_node=first, right_node=second, fq=first.fq + second.fq)
        first.p = parent
        second.p = parent
...
class Compress(object):
    ...
    def _get_tree_root(self):
        ...
        return tree_build(
            [Node_build(ch=ch, fq=int(fq)) for ch, fq in d.iteritems()]
            )
